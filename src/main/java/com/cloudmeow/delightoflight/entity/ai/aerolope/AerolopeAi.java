...
public class AerolopeAi {
    ...
    private static void initCoreActivity(Brain<AerolopeEntity> brain) {
        brain.addActivity(Activity.CORE, 0, ImmutableList.<BehaviorControl<? super AerolopeEntity>>of(new Swim(1.0F), new AnimalPanic(2.0F), new LookAtTargetSink(45, 90), new MoveToTargetSink(), new CountDownCooldownTicks(MemoryModuleType.TEMPTATION_COOLDOWN_TICKS), new CountDownCooldownTicks(MemoryModuleType.RAM_COOLDOWN_TICKS), new CountDownCooldownTicks(DFMemoryTypes.GROW_HORN_TICKS.get()), new CountDownCooldownTicks(DFMemoryTypes.SHAKE_HEAD_TICKS.get())));
    }

    private static void initIdleActivity(Brain<AerolopeEntity> brain) {
        brain.addActivityWithConditions(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0F, UniformInt.of(30, 60))), Pair.of(0, new AnimalMakeLove(DFEntityTypes.AEROLOPE.get())), Pair.of(1, new FollowTemptation((p_149446_) -> {
            return 1.25F;
