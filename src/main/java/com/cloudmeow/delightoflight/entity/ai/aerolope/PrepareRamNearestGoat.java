...
public class PrepareRamNearestGoat<E extends PathfinderMob> extends Behavior<E> {
    ...
    private Optional<PrepareRamNearestGoat.RamCandidate> ramCandidate = Optional.empty();

    public PrepareRamNearestGoat(ToIntFunction<E> p_147724_, int p_147725_, int p_147726_, float p_147727_, TargetingConditions p_147728_, int p_147729_, Function<E, SoundEvent> p_147730_) {
        super(ImmutableMap.of(MemoryModuleType.LOOK_TARGET, MemoryStatus.REGISTERED, MemoryModuleType.RAM_COOLDOWN_TICKS, MemoryStatus.VALUE_ABSENT, DFMemoryTypes.NEAREST_GOAT.get(), MemoryStatus.VALUE_PRESENT, MemoryModuleType.RAM_TARGET, MemoryStatus.VALUE_ABSENT), 160);
    ...
    protected void start(ServerLevel p_147736_, PathfinderMob p_147737_, long p_147738_) {
        ...
        brain.getMemory(DFMemoryTypes.NEAREST_GOAT.get()).flatMap(list -> {
            return list.stream()
                    .filter(target -> this.ramTargeting.test(p_147737_, target))
                    .min(Comparator.comparingDouble(target -> target.distanceToSqr(p_147737_)));
        }).ifPresent(nearestGoat -> {
            this.chooseRamPosition(p_147737_, nearestGoat);
        });
    }
    ...
    protected void tick(ServerLevel p_147784_, E p_147785_, long p_147786_) {
        if (!this.ramCandidate.isEmpty()) {
            ...
            if (flag) {
                ...
                this.chooseRamPosition(p_147785_, (this.ramCandidate.get()).getTarget());
            } else {
    ...
    private void chooseRamPosition(PathfinderMob p_147766_, Goat p_147767_) {
        this.reachedRamPositionTimestamp = Optional.empty();
        this.ramCandidate = this.calculateRammingStartPosition(p_147766_, p_147767_).map((p_289369_) -> {
            return new PrepareRamNearestGoat.RamCandidate(p_289369_, p_147767_.blockPosition(), p_147767_);
        });
    }
    ...
    public static class RamCandidate {
        ...
        public RamCandidate(BlockPos p_147794_, BlockPos p_147795_, Goat p_147796_) {
