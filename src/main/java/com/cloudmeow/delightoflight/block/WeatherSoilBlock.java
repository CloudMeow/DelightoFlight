...
public class WeatherSoilBlock extends RichSoilBlock {
    ...
    public BlockState getToolModifiedState(BlockState state, UseOnContext context, ItemAbility toolAction, boolean simulate) {
        return toolAction.equals(ItemAbilities.HOE_TILL) && context.getLevel().getBlockState(context.getClickedPos().above()).isAir() ? ((Block) DFBlocks.WEATHER_SOIL_FARMLAND.get()).defaultBlockState() : null;
    }
    ...
    public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource rand) {
        if (!level.isClientSide) {
            ...
            if (aboveBlock instanceof BonemealableBlock) {
                ...
                if ((double) MathUtils.RAND.nextFloat() <= (Double)Configuration.RICH_SOIL_BOOST_CHANCE.get() && growable.isValidBonemealTarget(level, pos.above(), aboveState) && CommonHooks.canCropGrow(level, pos.above(), aboveState, true)) {
